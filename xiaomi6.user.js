...
function newRow(no){
	...
	cti.val(config.seq[no].frequent);
	pi.change(function(){
		config.seq[no].text=$(this).val();
	});
	cti.change(function(){
		config.seq[no].frequent=$(this).val();
	});
	psi.val(config.seq[no].time);
	psi.change(function(){
...
function confirm(data){
	...
	if(document.location.href.indexOf("order.xiaomi.com/buy/checkout")>-1&&$("#checkoutToPay").length>0){
		...
		window.location.reload();
		return;
	}else if(document.location.href.indexOf("p.www.xiaomi.com/errors")>-1){
		window.location.href="http://order.xiaomi.com/cart/add/2135000541";
		return;
	}
	autoclick();
}
...
function mode4(){
	var element= $(cp.type).find(".egg-item");
	debugger;
	var eggIndex=Math.round(Math.random()*2);
	var egg=element[eggIndex];
	//element.each(function(){
	egg.click();
	changPage(++config.index);
	//	});
